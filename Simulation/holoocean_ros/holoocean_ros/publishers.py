...
class ImagePublisher:
    ...
    def publish_images(self, state, sim_time):
        ...
        if "Cam0DepthImg" in state:
            ...
            threshold = 0.99 * max_raw_depth
            valid_depth_mask = depth_img < threshold
            
            # Calculate what percentage of the image is at max depth
            max_depth_percentage = 1.0 - (np.sum(valid_depth_mask) / valid_depth_mask.size)
            ...
            # If most of the image (over 80%) is at max depth, apply extra filtering
            if max_depth_percentage > 0.8:
                # Increase confidence threshold drastically for images with mostly max depth
                confidence_threshold = 0.8  # Stricter threshold
                rospy.loginfo(f"High max-depth percentage detected ({max_depth_percentage:.2f}). Using stricter filtering.")
            else:
                # Normal confidence threshold 
                confidence_threshold = 0.1
            
            # Combine confidence threshold with valid depth mask
            confidence_mask = (confidence > confidence_threshold) & valid_depth_mask
            
            # Create filtered depth image (set depth values to NaN where confidence is too low or at max depth)
            ...
            if valid_points_percentage < 0.05:
    ...
    def publish_freespace_pointcloud(self, state, sim_time):
        ...
        if "Cam0DepthImg" in state:
            ...
            clip_depth = np.clip(depth_img, 0, 3000.0) / 100.0  # Convert to meter
