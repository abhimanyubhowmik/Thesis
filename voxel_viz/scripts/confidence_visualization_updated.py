    ...
    def visualize_cubes(self, event=None):
        ...
        try:
            ...
            if not results_copy:
                ...
                # FIXED: Don't clear markers if we have no data - keep existing ones
                return
            ...
            # FIXED: More conservative marker cleanup - only delete markers that haven't been seen for a while
            # Instead of deleting all old markers, only delete those from cells that have been cleaned up
            with self.data_lock:
                active_grid_cells = set(self.results_dict.keys())
            
            active_marker_ids = set()
            for idx in active_grid_cells:
                x_idx, y_idx, z_idx = idx
                marker_id = hash((x_idx, y_idx, z_idx)) % (2**31 - 1)
                if marker_id < 0:
                    marker_id = abs(marker_id)
            ...
            # Only delete markers that are no longer in our active results
            markers_to_delete = self.published_marker_ids - active_marker_ids
            ...
            # Update published marker IDs to include all active markers, not just current batch
            ...
            rospy.loginfo(f"Published {len(sorted_items)} cubes (total active: {len(active_grid_cells)}). Color distribution: {color_distribution}")
            if markers_to_delete:
                rospy.loginfo(f"Deleted {len(markers_to_delete)} old markers")
